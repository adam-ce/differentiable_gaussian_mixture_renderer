...
template<typename scalar_t>
STROKE_DEVICES_INLINE cuda::std::tuple<glm::vec<3, scalar_t>, glm::bvec3> sh_to_colour(const SHs<3, scalar_t>& sh, int deg, const glm::vec<3, scalar_t>& direction)
{
    // based on Differentiable Point-Based Radiance Fields for Efficient View Synthesis by Zhang et al. (2022)
    glm::vec<3, scalar_t> result = scalar_t(SH_C0) * sh[0];
    if (deg > 0) {
        scalar_t x = direction.x;
        scalar_t y = direction.y;
        scalar_t z = direction.z;
        result = result - scalar_t(SH_C1) * y * sh[1] + scalar_t(SH_C1) * z * sh[2] - scalar_t(SH_C1) * x * sh[3];
        ...
        if (deg > 1) {
            ...
            scalar_t yz = y * z;
            scalar_t xz = x * z;
            result += scalar_t(SH_C2[0]) * xy * sh[4]
                + scalar_t(SH_C2[1]) * yz * sh[5]
                + scalar_t(SH_C2[2]) * (2.0f * zz - xx - yy) * sh[6]
                + scalar_t(SH_C2[3]) * xz * sh[7]
                + scalar_t(SH_C2[4]) * (xx - yy) * sh[8];

            if (deg > 2) {
                result += scalar_t(SH_C3[0]) * y * (3.0f * xx - yy) * sh[9]
                    + scalar_t(SH_C3[1]) * xy * z * sh[10]
                    + scalar_t(SH_C3[2]) * y * (4.0f * zz - xx - yy) * sh[11]
                    + scalar_t(SH_C3[3]) * z * (2.0f * zz - 3.0f * xx - 3.0f * yy) * sh[12]
                    + scalar_t(SH_C3[4]) * x * (4.0f * zz - xx - yy) * sh[13]
                    + scalar_t(SH_C3[5]) * z * (xx - yy) * sh[14]
                    + scalar_t(SH_C3[6]) * x * (xx - 3.0f * yy) * sh[15];
            }
        }
    }
    result += scalar_t(0.5);
