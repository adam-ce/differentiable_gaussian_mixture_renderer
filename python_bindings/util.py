...
cuda_extra_cuda_cflags = ["-O3",  "--use_fast_math", "--expt-extended-lambda", "-std=c++20", f" {assert_flag} -DGLM_FORCE_LEFT_HANDED -DGLM_FORCE_QUAT_DATA_WXYZ --expt-relaxed-constexpr",  profiler_sync_extra]  # 
if platform.system() == "Windows":
    ...
    # cuda_extra_cflags = ["-O3", "-ffast-math", "-march=native", "--expt-extended-lambda --expt-relaxed-constexpr", "-std=c++20", profiler_sync_extra]
    cuda_extra_cflags = [f"-O3 -ffast-math -march=native --expt-extended-lambda --expt-relaxed-constexpr -std=c++20 {assert_flag} -DGLM_FORCE_XYZW_ONLY -DGLM_ENABLE_EXPERIMENTAL", profiler_sync_extra]
    cpp_extra_cflags = ["-fopenmp", "-ffast-math", " -fno-finite-math-only", "-O4", "-march=native", "--std=c++20", f" {assert_flag} -DGLM_FORCE_XYZW_ONLY -DGLM_ENABLE_EXPERIMENTAL", profiler_sync_extra]
    cuda_extra_cuda_cflags.append(f"-Xcompiler -fopenmp {profiler_sync_extra} {assert_flag} -DGLM_FORCE_XYZW_ONLY -DGLM_ENABLE_EXPERIMENTAL")
